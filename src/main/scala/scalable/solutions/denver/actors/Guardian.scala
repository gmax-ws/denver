...
object Guardian {
  ...
  def apply(): Behavior[GuardianCommand] = {
    Behaviors.setup[GuardianCommand] { context =>
      Behaviors.supervise(WebServer()).onFailure[Exception](SupervisorStrategy.restart)
      val webServer: ActorRef[WebServer.Command] = context.spawn(WebServer(), "http-server")
